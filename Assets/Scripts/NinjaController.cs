...
namespace NinjaController {
  ...
  public class NinjaController : MonoBehaviour {
    ...
    {
            speedMultiplier = Convert.ToSingle((StaticStats.numItem2 / 10.0) + 1);
    }
    ...
    private void ComputeVelocity(float dt) {
      ...
      if (isPlayerInAir) {
        currentVelocity.x = Mathf.Clamp(currentVelocity.x, -PhysicsParams.inAirMaxVelHorizontal * speedMultiplier, PhysicsParams.inAirMaxVelHorizontal * speedMultiplier);
      } else {
        currentVelocity.x = Mathf.Clamp(currentVelocity.x, -PhysicsParams.onGroundMaxVelHorizontal * speedMultiplier, PhysicsParams.onGroundMaxVelHorizontal * speedMultiplier);
      }


      //switch direction player is facing based on velocity
      if (currentVelocity.x < 0)
            {
                transform.localScale = new Vector3(-1f * Mathf.Abs(transform.localScale.x), transform.localScale.y, transform.localScale.z);
                aimLead.transform.localScale = new Vector3(-1f * Mathf.Abs(aimLead.transform.localScale.x), aimLead.transform.localScale.y, aimLead.transform.localScale.z);
            }
            else if (currentVelocity.x > 0)
            {
                transform.localScale = new Vector3(Mathf.Abs(transform.localScale.x), transform.localScale.y, transform.localScale.z);
                aimLead.transform.localScale = new Vector3(Mathf.Abs(aimLead.transform.localScale.x), aimLead.transform.localScale.y, aimLead.transform.localScale.z);
            }

      RBody.velocity = currentVelocity;
    }
        bool jumpKeyLifted = true;
    private void ProcessInput() {
      ...
      if (isPlayerInAir == false) {
        ...
        if (isKeyDownJump == true && keyJumpRetrigger) {     //if (isKeyDownJump == true && keyJumpRetrigger == true) FIXME IN CASE BROKEN
          keyJumpPressed = true;
                    //currentJumps++;


          keyJumpRetrigger = false;    //keyJumpRetrigger = false;
                    jumpKeyLifted = false;
      ...
      } else if (isKeyDownJump == true && currentJumps< (StaticStats.numItem1) && jumpKeyLifted)
            {
                currentJumps++;
      ...
      if (isPlayerInAir == true) {
        ...
        if (isKeyDownLeft == true && currentVelocity.x <= 0)
          SimAddForce(new Vector2(-PhysicsParams.inAirMoveHorizontalForce * speedMultiplier, 0) * EntityMass);
        ...
        //steering against moving direction (fast reverse accel)
          ...
          SimAddForce(new Vector2(-PhysicsParams.inAirMoveHorizontalForceReverse * speedMultiplier, 0) * EntityMass);
        else if (isKeyDownRight == true && currentVelocity.x <= 0)
          SimAddForce(new Vector2(PhysicsParams.inAirMoveHorizontalForceReverse * speedMultiplier, 0) * EntityMass);
      }
      ...
      if (isPlayerInAir == false) {
        //steering into moving direction (slow accel)
        if (isKeyDownLeft == true && currentVelocity.x <= 0)
          SimAddForce(new Vector2(-PhysicsParams.onGroundMoveHorizontalForce * speedMultiplier, 0) * EntityMass);
        else if (isKeyDownRight == true && currentVelocity.x >= 0)
          SimAddForce(new Vector2(PhysicsParams.onGroundMoveHorizontalForce * speedMultiplier, 0) * EntityMass);
        //steering against moving direction (fast reverse accel)
        else if (isKeyDownLeft == true && currentVelocity.x >= 0)
          SimAddForce(new Vector2(-PhysicsParams.onGroundMoveHorizontalForceReverse * speedMultiplier, 0) * EntityMass);
        else if (isKeyDownRight == true && currentVelocity.x <= 0)
          SimAddForce(new Vector2(PhysicsParams.onGroundMoveHorizontalForceReverse * speedMultiplier, 0) * EntityMass);
